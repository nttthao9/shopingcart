...
window.addEventListener('load', function () {
  ...
  function creatItemBuy(src, itemName, itemPrice) {
    ...
    cart.insertAdjacentHTML('beforbegin', itemBuy);
  }
  ...
  for (let i = 0; i < cartIcon.length; i++) {
    cartIcon[i].addEventListener('click', function (e) {
      ...
      if (productCount[i].value > 0) {
        ...
        creatItemBuy(src, productName[i], productPrice[i]);
        console.log(productName[i], productPrice[i]);
      }
    });
  }
});
